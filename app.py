...
def read_tensor_from_image(image, file_name, input_height=299, input_width=299, input_mean=0, input_std=255):
  input_name = "file_reader"
  # output_name = "normalized"
...
def upload2():
    if request.method == 'POST':
        ...
        with tf.Session(graph=new_graph) as sess:
          start = time.time()
          results = sess.run(new_output_operation.outputs[0],
                            {new_input_operation.outputs[0]: t})
          end=time.time()
        results = np.squeeze(results)

        top_k = results.argsort()[-5:][::-1]
        labels = load_labels(new_label_file)
...
if __name__ == '__main__':
    ...
    new_model_file = "new_retrained_graph.pb"
    new_label_file = "new_retrained_labels.txt"
    input_height = 224
    input_width = 224
    new_input_height = 299
    new_input_width = 299
    input_mean = 128
    input_std = 128
    input_layer = "input"
    ...
    new_graph = new_load_graph(new_model_file)

    # Grab the Input/Output operations
    input_name = "import/" + input_layer
    output_name = "import/" + output_layer
    new_input_name = "import/Mul"
    new_output_name = "import/final_result"
    input_operation = graph.get_operation_by_name(input_name)
    new_input_operation = new_graph.get_operation_by_name(new_input_name)
    output_operation = graph.get_operation_by_name(output_name)
    new_output_operation = new_graph.get_operation_by_name(new_output_name)
